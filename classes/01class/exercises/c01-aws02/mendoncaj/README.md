...
```
1) it was hard to join the pieces to create the role and the instance profile allowing ec2 instances to access non public s3 objects.

2) a lot of steps to provide a way to communicate a private instance (that resides into a private subnet) to access to the internet and consecutively accessing s3 objects.
```
