...
3. Verify Application built by adding an item to DynamoDB through API Gateway
   curl -v --location --request POST 'https://kxvan3zgik.execute-api.ap-southeast-2.amazonaws.com/test/customers' --header 'Content-Type: application/json' --header "x-api-key: Jf65mYBKen2IKoNckN2323QdEOwr6JvZ1eFXXJWG" --data-raw '{"firstname": "Maya", "lastname": "Vj", "email": "mayavj@y.com"}'
```
