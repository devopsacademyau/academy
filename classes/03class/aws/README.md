...
This article will cover why you should be chosing CATTLE over PETS and how to implement.


## PET vs CATTLE

The analogy comes from how do you treat your servers. In the old days, you would even give a name to your server like, Thunder/Flash or Tiger/Monkey if you prefer animal names. Very much like a PET where you need to individually take care of it, install programs, patch things, etc. These days with automation, we now treat servers as cattle, where if one server dies, we just replace it.

The idea is to avoid pet server as manual configuration tend to generate more maintenance.

Pet servers have issues with scaling up and down. For example a server with 8GB of RAM memory hosting an e-commerce, if you are getting more clients than what you expect, you will have to shutdown the computer, install more RAM and start again.
...
EC2 Auto Scaling ensures that you always have the amount of instances needed to handle the load of your application. Based on healthchecks and resources consumption logs, the asg will create new instances or shutdown the ones that are iddle.
...
There are no costs of creating an ASG by itself. The cost will be related with the number and type of EC2 instances that your group create.
...
If those instances can be deleted and restarted at any time to maintaint the amount of instances for the group. How would I keep all the configurations on it?
...
Each listener rule consists in a priority of actions and one or more conditions. When the conditions for a rule are met, the traffic is then forwarded to a target group.
...
If your service is being exposed through an ALB on port 443, a SSL certificate will need to be attached to the listener, otherwise when your clients try to access your webserver, their browser will complain about the connection not being safe.

AWS offers a service for generating SSL certificates for free if you are using with their managed services like a load balancer.

Read more about AWS ACM and SSL certificates [here](https://aws.amazon.com/certificate-manager/getting-started/?nc=sn&loc=7)

## Route53

R53 is the DNS managed service offered by AWS. To consume this service you first will need to create either private or public hosted zones.
